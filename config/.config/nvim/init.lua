...
-- Since the AI agent modifies files in the background while we might be looking at them,
-- we need to poll the filesystem.
timer:start(
  1000, -- delay
  1000, -- repeat every 1000ms
  vim.schedule_wrap(function()
    -- Only run checktime if the current buffer is a normal file.
    -- This avoids errors or flickering in special buffers like Telescope, NvimTree, or Terminals.
...
vim.api.nvim_create_autocmd('FileChangedShellPost', {
  ...
  callback = function()
    ...
    if vim.bo.modified then
      vim.notify('File changed on disk (by AI?) but you have unsaved changes!\nUse :DiffOrig to review differences.', vim.log.levels.WARN)
    else
      -- If the buffer was clean, autoread successfully reloaded it.
      -- We stay silent to avoid unnecessary distractions.
      -- Uncomment the line below if you want a subtle confirmation message:
      -- vim.notify('File updated externally (reloaded automatically).', vim.log.levels.INFO)
    end
  end,
})

-- VSCode-like diff command
